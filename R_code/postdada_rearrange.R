...
setwd("/home/bpalmer/Documents/GitHub/mad4hatter/work/cd/01b3af703309cd336e3df3246730f9")
# # FOR DEBUGGING
args <- list()
...
allele.data <- df_aln %>%
  dplyr::mutate(pseudo_cigar = mapply(build_pseudoCIGAR_string, masked_aligned_nodashinN_refseq, hapseq)) %>%
  ...
  select(sampleID, locus, hapseq, reads, allele, pseudo_cigar) %>%
  group_by(sampleID, locus, hapseq) %>%
  mutate(reads=sum(reads)) %>%
  dplyr::rename(asv=hapseq) %>%
  distinct()
