...
def helpMessage() {
  log.info """
    ...
    Examples:
      Change output directory and use custom config file for extra resources
      nextflow run main.nf --readDIR path/to/read_dir --outDIR results --pools D1,R1,R2 -config custom.config

      Change runtime profile to use Docker
      nextflow run main.nf --readDIR path/to/read_dir --pools D1,R1,R2 -profile docker
      
      Only run the QC workflow
      nextflow run main.nf --readDIR path/to/read_dir --pools D1,R1,R2 --workflow_name qc
      
      Only run the Postprocessing workflow
      nextflow run main.nf --workflow_name postprocessing --pools D1,R1,R2 --denoised_asvs results/raw_dada2_output/dada2.clusters.txt
      
      Alter Dada2 params 
      nextflow run main.nf --readDIR path/to/read_dir --pools D1,R1,R2 --omega_a 1e-40 --dada2_pool false --band_size 20 --maxEE 4
      
      Set full genome 
      nextflow run main.nf --readDIR path/to/read_dir --pools D1,R1,R2 --genome data/reference/v1/PkPfPmPoPv.fasta
      
      Set targeted reference 
      nextflow run main.nf --readDIR path/to/read_dir --pools D1 --refseq_fasta panel_information/D1.1/D1.1_reference.fasta
      
      Alter Masking parameters 
      nextflow run main.nf --readDIR path/to/read_dir --pools D1,R1,R2 --homopolymer_threshold 2 --trf_min_score 30 --trf_max_period 5
        """.stripIndent()
}
