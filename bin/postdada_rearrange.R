...
if (!is.null(args$homopolymer_threshold) && args$homopolymer_threshold > 0) {
  allele.data <- foreach(seq2cigar = 1:nrow(df_aln), .combine = "rbind") %dopar% {
      ...
      ranges_pseudocigar[["N"]]<-masked_ranges
          ...
          for (cigar_key in names(ranges_pseudocigar)) {
            if (length(ranges_pseudocigar[[cigar_key]]) > 0) {
              for (jj in 1:length(ranges_pseudocigar[[cigar_key]])) {
                bs.cigar[ranges_pseudocigar[[cigar_key]][jj]] <- c(cigar_key)
              }
            } else {
              bs.cigar[ranges_pseudocigar[[cigar_key]]] <- c(cigar_key)
            }
          }
