library(testthat)
library(tidyverse)
# source("../../bin/build_pseudocigar.R")
...
test_build_pseudoCIGAR_string <- function(build_pseudoCIGAR_string_func) {
  ...
  test_that("PseudoCIGAR has substitutions", {
    reference <- "ACTCCG"
    query <- "ACTTCG"
    expected_output <- "4T"
    result <- build_pseudoCIGAR_string_func(reference, query)
  ...
  test_that("PseudoCIGAR has multiple substitutions", {
    reference <- "ACTCCGAATC"
    query <- "ACTTCGATTC"
    expected_output <- "4T8T"
    result <- build_pseudoCIGAR_string_func(reference, query)
    expect_equal(result, expected_output)
  })

  test_that("PseudoCIGAR has substituions in masked regions", {
    reference <- "ANNNNNNNNC"
    query <- "ACTTCGATTC"
    expected_output <- "2+8N"
    result <- build_pseudoCIGAR_string_func(reference, query)
    expect_equal(result, expected_output)
  })

  test_that("PseudoCIGAR has substituions in masked regions (plus indels, making sure indel reporting in masked region unchanged)", {
    reference <- "ANNNNNNNNC"
    query <- "ACTT-GATTC"
    expected_output <- "2+8N"
    result <- build_pseudoCIGAR_string_func(reference, query)
  ...
  test_that("PseudoCIGAR has substituions with other mutations", {
    reference <- "ACTCCGAAT-C"
    query <- "ACTT-GATTGC"
    expected_output <- "4T5D=C8T10I=G"
    result <- build_pseudoCIGAR_string_func(reference, query)
    expect_equal(result, expected_output)
  })
